...
FK-003 and FK-002 were developed in parallel worktrees and both initially
claimed the next-available migration number at branch time, producing a
collision (FK-003 as `027_task_dependencies.sql`, colliding with FIX-003's
already-merged `027_epic_status_default.sql`, and ordering incorrectly
relative to FK-002's `028_task_fk_constraints.sql`). During integration
this was renumbered to `029_task_dependencies.sql` — **not just for the
collision**, but because ordering matters here: FK-002's migration does a
full `tasks` table rebuild whose `INSERT INTO tasks_new SELECT ...` column
list explicitly includes `depends_on` (it had to, since FK-002 only added
`REFERENCES` clauses to `sprint_id`/`project_id`/`epic_id`, not touch
`depends_on`). If this migration ran *after* FK-003 had already dropped
`depends_on`, the rebuild's `SELECT` would reference a nonexistent column
and fail. So FK-002 (028) must run before FK-003 (029) — confirmed by
reading both migration SQL files directly, not assumed. PRIM-004's
`archived_at` migration was similarly renumbered to `030` (was also `027`,
independent of this ordering constraint since it touches unrelated
tables). `migrate_test.go` was updated to assert the correct 028-then-029
sequencing (`depends_on` still present after 028, dropped after 029).

### Migration

`internal/persistence/sqlstore/migrations/029_task_dependencies.sql`,
...
- `internal/persistence/sqlstore/migrations/029_task_dependencies.sql` — new migration.
- `internal/persistence/sqlstore/migrations/migrate_test.go` — schema-shape assertions for 029.
