...
8. Real CLI executor smoke test: same flow, using a shell-script profile that emits `TORQUE_DONE`. (Covers executor plumbing but no artifact gating beyond Component 5's fix.)
...
```
TORQUE_ARTIFACT {"type":"diff","content":"--- a/x\n+++ b/x\n@@","url":"","file_path":""}
```
...
### Step 6 — Scheduler integration into `torque serve` (the big one)
...
**Files:**
- `cmd/torque/serve.go` — substantial rewrite of the run path
...
**Tests:**
- `cmd/torque/serve_test.go` — probably doesn't exist today; skip if too expensive to add. Covered by integration tests instead.
...
- **New:** `cmd/torque/serve_e2e_test.go` — spins up `serve` in a goroutine, hits `POST /api/v1/tasks`, polls `GET /api/v1/tasks/{id}` until `done`, asserts the transition happened. This is the true smoke test.
...
- `docs/examples/profiles/smoke-echo.sh` (new) — shell script that echoes `TORQUE_DONE` (and optionally a `TORQUE_ARTIFACT` signal)
- `cmd/torque/smoke_cli_test.go` (new) — spins up `serve`, creates a task pointing at this profile, polls to completion
...
echo 'TORQUE_ARTIFACT {"type":"log","content":"smoke run complete"}'
echo "TORQUE_DONE"
```
...
- `cmd/torque/serve_e2e_test.go` — NEW, HTTP POST + poll to completion
- `cmd/torque/smoke_cli_test.go` — NEW, real CLI executor smoke
...
| Queue.db file locked if both `torqued` and `serve` run | **Action:** Document that `torqued` is deprecated; remove it in the same PR as Step 6. Prevents accidental concurrent boot. |
| Cerberus currently manages `torque-api` and `torque-frontend` but no `torqued` | No action — Cerberus already doesn't run `torqued`. `torque serve` is what it launches. Good luck. |
...
- `cmd/torqued/main.go` — redundant after Step 6. Scheduler lives in `serve`; `torqued` is dead code.
- Any Cerberus / Makefile targets referencing `torqued` — check and update.
- Stub response strings in `internal/httpserver/settings.go` ("Scheduler not yet implemented — see Plan 2")

**Confirm before deleting:** Is there any external expectation (docs, tests, deploy scripts) that `torqued` is a separate binary? If yes, deprecate first, delete in Path B.
...
3. Manual smoke: `torque serve` booted, `curl POST /api/v1/tasks` with mock executor, poll `GET /api/v1/tasks/{id}` until `done`, observe the transition in ≤2s.
...
**Branch:** `feat/e2e-execution` (already created as worktree at `../torque-backend-e2e`).
