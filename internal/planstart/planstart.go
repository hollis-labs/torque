...
func Redispatch(ctx context.Context, store RedispatchStore, mgr SessionManager, planID string, opts Options) (res *Result, err error) {
	...
	// further down).
	//
	// priorSession captures the EXITED-or-stale orchestrator session row so
	// the cold-boot recovery pack below can replay its trailing turns into
	// the freshly-booted orchestrator. This is torque's
	...
	if existing, ok := readOrchestratorSessionID(plan); ok && existing != "" {
		if rec, err := store.GetSession(existing); err == nil {
			if !sessionTerminal(rec.State) && mgr.IsAlive(existing) {
				return &Result{
					SessionID: existing,
					PlanID:    planID,
					StartedAt: rec.CreatedAt,
			...
			// Terminal, or stale non-terminal (row says running/launching but
			// the manager's registry disagrees) — eligible for recovery replay.
			priorSession = rec
		}
