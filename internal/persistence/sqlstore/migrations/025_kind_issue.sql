-- 025 widen tasks.kind CHECK to include 'issue'.
--
-- Issues are project-scoped backlog capture rows stored in tasks as
-- kind='issue', status='backlog', manual=true. They are intentionally
-- non-dispatching; the scheduler skips them like plan/parent coordination
-- rows. SQLite cannot ALTER a CHECK constraint, so rebuild the table.
