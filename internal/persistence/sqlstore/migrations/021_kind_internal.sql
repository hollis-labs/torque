-- 021 widen tasks.kind CHECK to include 'internal' (CW-20260503-0011, S1.1).
--
-- 'internal' is the substrate primitive for automation/system tasks
-- (Reviewer end-agents, future System / Project Manager agents, etc.) that
-- participate in the scheduler but stay out of user-facing list views and
-- per-project concurrency accounting.
--
-- SQLite cannot ALTER a CHECK constraint; full table rebuild required. The
-- column list reproduces 016 (executor default 'opencode') plus the three
-- columns 020 added via ALTER (collection_id, collection_position,
-- added_to_collections_at). FK references from sibling tables (task_tags,
