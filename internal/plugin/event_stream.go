...
func (h *TorqueHost) SubscribeEvents() chan goplugin.Event {
...
func (h *TorqueHost) UnsubscribeEvents(ch chan goplugin.Event) {
	h.mu.Lock()
	defer h.mu.Unlock()
...
func (h *TorqueHost) broadcastEvent(event goplugin.Event) {
	h.mu.RLock()
